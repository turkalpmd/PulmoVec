...
def prepare_features_and_labels(df, outcome_name):
    """
    Prepare features (11 total) and labels for a specific outcome.
    ...
    Returns:
        X: Feature DataFrame [N, 11] (with column names for LightGBM)
        y: Label array [N]
    ...
    # Extract demographic features (3)
    ...
    # Extract recording_location feature (1)
    # Encode recording_location using LabelEncoder
    if 'recording_location' in df_encoded.columns:
        location_encoder = LabelEncoder()
        # Handle missing values by filling with most frequent
        if df_encoded['recording_location'].isna().any():
            most_frequent = df_encoded['recording_location'].mode()[0]
            df_encoded['recording_location'] = df_encoded['recording_location'].fillna(most_frequent)
            logging.warning(f"Filled {df_encoded['recording_location'].isna().sum()} missing recording_location values with most frequent: {most_frequent}")
        
        # Encode recording_location
        df_encoded['recording_location_encoded'] = location_encoder.fit_transform(df_encoded['recording_location'])
        logging.info(f"Encoded recording_location: {len(location_encoder.classes_)} unique locations")
        location_feature = 'recording_location_encoded'
    else:
        logging.warning("recording_location column not found in DataFrame. Skipping this feature.")
        location_feature = None
    
    # Combine all features (11 total if recording_location exists, 10 otherwise) - Keep as DataFrame for feature names
    feature_cols = prob_cols + ['age', 'gender_encoded']
    if location_feature:
        feature_cols.append(location_feature)
...
def evaluate_model_detailed_with_ci95(model, X_val, y_val, outcome_name, label_encoder=None, n_bootstrap=1000):
    ...
    for i, class_name in enumerate(class_names):
        ...
        def f1_func(y_true, y_pred):
            _, _, f1, _ = precision_recall_fscore_support(y_true, y_pred, labels=[i], zero_division=0)
            return f1[0] if len(f1) > 0 else 0.0
        metrics['f1_per_class'][class_name] = bootstrap_metric(y_val, y_pred, None, f1_func, n_bootstrap)
...
def plot_roc_auprc_curves(y_true, y_proba, class_names, outcome_name, save_path):
    """
    Plot ROC and Precision-Recall curves (one-vs-rest) and save as PNG.

    This reproduces the same artifact style used in existing
    `roc_auprc_curves.png` files under meta_model results.
    """
    n_classes = len(class_names)
    old_style = plt.rcParams.copy()
    plt.style.use("ggplot")
    # Wider figure as requested (width=18, height=10)
    fig, axes = plt.subplots(1, 2, figsize=(18, 10))

    # Nice display name for title
    display_map = {
        "model4_label": "Disease Group Prediction Model",
        "disease": "Disease Classification Model",
        "event_type": "Event Type Meta-Model",
    }
    display_name = display_map.get(outcome_name, str(outcome_name))

    # ggplot-like categorical palette for consistent class coloring.
    colors = ["#E24A33", "#348ABD", "#988ED5", "#777777", "#FBC15E", "#8EBA42", "#FFB5B8"]

    # ROC panel
    for i, class_name in enumerate(class_names):
        y_true_bin = (y_true == i).astype(int)
        # Skip degenerate class slices safely
    ...
    axes[0].set_title("ROC (One-vs-Rest)", fontsize=14, fontweight="bold")
    axes[0].set_xlabel("False Positive Rate", fontsize=14)
    axes[0].set_ylabel("True Positive Rate", fontsize=14)
    axes[0].tick_params(axis="both", labelsize=14)
    axes[0].grid(alpha=0.3)
    axes[0].legend(fontsize=12, loc="lower right", framealpha=0.95)
    ...
    axes[1].set_title("Precision-Recall (One-vs-Rest)", fontsize=14, fontweight="bold")
    axes[1].set_xlabel("Recall", fontsize=14)
    axes[1].set_ylabel("Precision", fontsize=14)
    axes[1].tick_params(axis="both", labelsize=14)
    axes[1].grid(alpha=0.3)
    axes[1].legend(fontsize=14, loc="lower left", framealpha=0.95)

    fig.suptitle(
        f"{display_name} – Multi-Class Performance Curves",
        fontsize=16,
        fontweight="bold",
    )
    # Bring plots closer to title slightly
    plt.tight_layout(rect=[0, 0.04, 1, 0.94])
    plt.savefig(save_path, dpi=300, bbox_inches="tight")
    plt.close()
    plt.rcParams.update(old_style)
    logging.info(f"  ✓ Saved ROC/AUPRC curves: {save_path.name}")
...
"""
    ...
    for class_name in class_names:
        ...
        # F1-Score with CI95
        f1_str = f"{f1['value']:.4f} [{f1['ci95_lower']:.4f}, {f1['ci95_upper']:.4f}]"
        
        report += f"| {class_name} | {ppv['value']:.4f} [{ppv['ci95_lower']:.4f}, {ppv['ci95_upper']:.4f}] | {sens['value']:.4f} [{sens['ci95_lower']:.4f}, {sens['ci95_upper']:.4f}] | {f1_str} | {spec['value']:.4f} [{spec['ci95_lower']:.4f}, {spec['ci95_upper']:.4f}] | {npv['value']:.4f} [{npv['ci95_lower']:.4f}, {npv['ci95_upper']:.4f}] | {support} | {roc_auc_str} |\n"
...
def main():
    ...
    for outcome_name, outcome_info in outcomes.items():
        ...
        # Use validation probabilities directly so plots reflect final model outputs.
        y_proba_val = model.predict_proba(X_val)
        plot_roc_auprc_curves(
            y_true=y_val,
            y_proba=y_proba_val,
            class_names=class_names,
            outcome_name=outcome_name,
            save_path=outcome_dir / "roc_auprc_curves.png",
        )
