...
class SPRSoundDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, int, Dict]:
        ...
        # Resolve relative paths against PROJECT_ROOT so scripts can be run
        # from any working directory (e.g. Projects/ or Projects/scripts/).
        wav_path = Path(row['wav_path'])
        if not wav_path.is_absolute():
            wav_path = config.PROJECT_ROOT / wav_path
